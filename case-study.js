(function (global) {
  ...
  function buildSlider(p_data, sliderEl, activeIndex) {
    ...
    var slides = allProjects.map(function (pr) {
      ...
      return { src: src, optimizedSrc: (pr.media && pr.media.heroAvif) || '', caption: caption, name: pr.subtitle || pr.title || '',
    ...
    slides.forEach(function (slide, i) {
      ...
      card.className = 'proj-card ' + slide.shape + (slide.src ? ' is-loading' : ' is-empty');
      card.setAttribute('aria-busy', slide.src ? 'true' : 'false');

      // Neutral fallback remains behind the skeleton/image.
      ...
      if (slide.src) {
        ...
        if (slide.optimizedSrc) {
          var source = document.createElement('source');
          source.type = 'image/avif';
          source.setAttribute('data-srcset', slide.optimizedSrc);
    ...
    function hydrateSlide(index, highPriority) {
      index = (index + TOTAL) % TOTAL;
      var card = slideEls[index] && slideEls[index].querySelector('.proj-card');
      ...
      trackSkeletonImage(img, card);
      if (source) source.srcset = source.getAttribute('data-srcset');
      img.src = img.getAttribute('data-src');
    }

    function hydrateSlideWindow(index) {
      hydrateSlide(index, true);
      hydrateSlide(index - 1, false);
      hydrateSlide(index + 1, false);
    }

    hydrateSlideWindow(activeIndex);
  ...
  function buildDetail(p_data, detail) {
    ...
    // ── App link — only render when the project has a live destination ──
    if (p_data.sourceUrl && p_data.sourceUrl.trim()) {
    ...
    if (gallerySections.length) {
      ...
      gallerySections.forEach(function (section, sectionIndex) {
        ...
        items.forEach(function (item) {
          ...
          var frame = div('cs-g-frame' + (item.src ? ' is-loading' : ' is-empty'));
          frame.setAttribute('aria-busy', item.src ? 'true' : 'false');
          var ph = div('cs-g-ph');
          ph.setAttribute('aria-hidden', 'true');
